...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        ...
        ListaConPI<EntradaHash<C, V>>[] elArrayViejo = this.elArray;
        ...
        talla = 0;
        for (int i = 0; i < elArrayViejo.length; i++) {
            ListaConPI<EntradaHash<C, V>> cubeta = elArrayViejo[i];
    ...
    public final double desviacionTipica() {
        ...
        for(int i = 0; i < elArray.length ; i++) {
                double tam = elArray[i].talla();
                sumatorio = sumatorio + ((tam - media) * (tam - media));
        }
        
        double res = Math.sqrt(sumatorio / elArray.length);
        return res;
    }
    ...
    public final double costeMLocalizar() {
        ...
        int colisiones = 0;
        for (int i = 0; i < elArray.length; i++) {
            colisiones = colisiones + elArray[i].talla() * (1 + elArray[i].talla()) / 2;
        }
