...
public abstract class Grafo {
    ...
    protected void arbolRecubrimientoBFS(int origen, Arista[] res) { 
        ...
        while (!q.esVacia()) {
            ...
            for (l.inicio(); !l.esFin(); l.siguiente()) {
                Adyacente a = l.recuperar(); 
                if (visitados[a.destino] == 0) {
                    Arista nueva = new Arista(u, a.destino, a.peso);
                    res[ordenVisita] = nueva;
                    ordenVisita++;
                    visitados[a.destino] = 1;
                    q.encolar(a.destino);
                }
    ...
    public Arista[] kruskal() {       
        ...
        for(int i = 0; i < numVertices(); i++) {
            ListaConPI<Adyacente> l = this.adyacentesDe(i);
            for(l.inicio(); !l.esFin(); l.siguiente()) {
                Adyacente a = l.recuperar();
        ...
        while(cardinalE < numVertices() - 1 && !aristasFactibles.esVacia()) {
            ...
            int ccV = cc.find(a.getOrigen());
            int ccW = cc.find(a.getDestino());
            if(ccV != ccW) {
                res[cardinalE] = a;
                cardinalE++;
                cc.union(ccV, ccW);
